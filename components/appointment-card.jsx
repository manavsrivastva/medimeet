...
const AppointmentCard = ({ appointment, userRole }) => {
    ...
    const otherParty = 
      ...
      const handleMarkCompleted = async()=>{
        ...
        if(
          window.confirm(
            "Are you sre you want to mark this appointment as completed? This action cannot be undone"
          )
        ){
      ...
      useEffect(()=> {
        if(tokenData?.success){
          router.push(
  `/video-call?sessionId=${tokenData.videoSessionId}&token=${encodeURIComponent(tokenData.token)}&appointmentId=${appointment.id}`
);
      ...
      const handleCancelAppointment = async()=>{
        ...
        if(
          window.confirm(
            "Are you sre you want to cancel this ? This action cannot be undone"
          )
        ){
          const formData = new FormData();
          formData.append("appointmentId" , appointment.id);
      ...
      useEffect(() => {
        if(cancelData?.success){
          toast.success("Appointment cancelled successfuly");
          setOpen(false);
        }
      } , [cancelData]) ; 
        ...
        <div className='flex flex-col md:flex-row justify-between gap-4'>
          ...
          <div className='flex flex-col gap-2 self-end md:self-start'>
            ...
            className={ 
              ...
              ? "bg-emerald-900/20 border-emerald-900/30 text-emerald-400 self-start" : appointment.status === "CANCELLED"
              ...
              <div className='space-y-2'>
                ...
                >
                  ...
                  {tokenLoading || action === "video" ?  (
                    ...
                    {isAppointmentActive()  
                    ?
                  "Join Video Call" : "Video call will be available 30minutes before appointment"}
                    </>
                  )}
